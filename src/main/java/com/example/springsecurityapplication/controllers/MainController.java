...
//    Получаем объект аутентификации -> с помощью SpringContextHolder обращаемся к контексту и на нем вызываем метод аутентификации. Из сессии текущенго пользователя получаем объект, который был положен в данную сессию после аутентификации пользователя
...
//    1 способ работы с моделью через Model
...
//   2 способ работы с моделью через @ModelAttribute
    @GetMapping("/registration")
...
//  получаем товары из корзины по id товара
        ...
        cartRepository.deleteCartByPersonId(id);
        return "redirect:/cart";
    }
//  обработка заказов
...
//  получаем товары из корзины по id товара
        ...
        for (Product product : productList){
            ...
            cartRepository.deleteCartByPersonId(product.getId());
        }
